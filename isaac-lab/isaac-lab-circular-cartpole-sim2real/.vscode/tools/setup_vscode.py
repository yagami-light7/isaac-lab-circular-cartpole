...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    if os.path.exists(isaacsim_vscode_filename):
        ...
        # On Windows, check if both paths are on the same drive before computing relative path
        try:
            ...
            # Different drives on Windows - use absolute path instead
            abs_path = os.path.normpath(ISAACSIM_DIR).replace("\\", "/")
            path_names = [
                '"' + abs_path + "/" + path_name + '"' for path_name in path_names
            ]
    else:
        path_names = []
        print(
    ...
    # Use a lambda to avoid regex interpretation of backslashes in the replacement string
...
def overwrite_default_python_interpreter(isaaclab_settings: str) -> str:
    ...
    # Normalize path separators for use in JSON (replace backslashes with forward slashes)
    python_exe = python_exe.replace("\\", "/")
    ...
    # Use a lambda to avoid regex interpretation of backslashes in the replacement string
    replacement = f'"python.defaultInterpreterPath": "{python_exe}"'
    isaaclab_settings = re.sub(
